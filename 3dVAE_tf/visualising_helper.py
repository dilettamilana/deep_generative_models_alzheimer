...
class Visualiser():
    ...
    def calculate_silhouette(self, X, labels, title):
    ...
    def mmse_scatter(self, x, y, labels, imageData, zoom, variable_for_scatter):
        ...
        x_reshaped = np.reshape(x, newshape=[len(x), 1])
        y_reshaped = np.reshape(y, newshape=[len(y), 1])
        features_for_clustering = np.concatenate((x_reshaped, y_reshaped), axis=1)
        clustering_score = self.calculate_silhouette(features_for_clustering,
                                                     labels[0:x.shape[0], variable_for_scatter], title="ad_nc_mci")

        print("MMSE pearson correlation")
        pearson_correlation_1 = scipy.stats.pearsonr(x, labels[0:x.shape[0], variable_for_scatter])
        print("pearson correlation x")
        print(pearson_correlation_1)
        pearson_correlation_2 = scipy.stats.pearsonr(y, labels[0:x.shape[0], variable_for_scatter])
        ...
        plt.title(title+" scatter plot| clustering score: " +str(clustering_score))
    ...
    def sex_scatter(self, x, y, labels, imageData, zoom, variable_for_scatter):
        ...
        clustering_score = self.calculate_silhouette(features_for_clustering,
                                                     labels[0:x.shape[0], variable_for_scatter], title="ad_nc_mci")
        print("GENDER pearson correlation")
        pearson_correlation_1 = scipy.stats.pearsonr(x, labels[0:x.shape[0], variable_for_scatter])
        print("pearson correlation x")
        print(pearson_correlation_1)
        pearson_correlation_2 = scipy.stats.pearsonr(y, labels[0:x.shape[0], variable_for_scatter])
    ...
    def aibl_oasis_harp_scatter(self, x, y, labels, imageData, zoom, variable_for_scatter):
        ...
        print("AIBL OASIS HARP pearson correlation")
        pearson_correlation_1= scipy.stats.pearsonr(x, labels[0:x.shape[0], variable_for_scatter])
        print("pearson correlation x")
        print(pearson_correlation_1)
        pearson_correlation_2 = scipy.stats.pearsonr(y, labels[0:x.shape[0], variable_for_scatter])
    ...
    def ad_nc_mci_scatter(self, x, y, labels, imageData, zoom, variable_for_scatter):
        ...
        y_reshaped = np.reshape(y, newshape=[len(y), 1])
        features_for_clustering= np.concatenate((x_reshaped, y_reshaped), axis=1)
        clustering_score= self.calculate_silhouette(features_for_clustering, labels[0:x.shape[0], variable_for_scatter], title="ad_nc_mci")
        print("AD-NC-MCI pearson correlation")
        pearson_correlation_1= scipy.stats.pearsonr(x, labels[0:x.shape[0], variable_for_scatter])
        print("pearson correlation x")
        print(pearson_correlation_1)
        pearson_correlation_2 = scipy.stats.pearsonr(y, labels[0:x.shape[0], variable_for_scatter])
        ...
        plt.title(title + " scatter plot | pearson x: " +str(pearson_correlation_1)+", pearson y: "+str(pearson_correlation_2))
        plt.savefig(self.directory + "scatter_" + str(title) + ".png")
        plt.close()
...
# plt.show()
    ...
    def plot_progression(self, dataset_images, dataset_encoded_images, dataset_labels, epoch):
        ...
        for patient in patient_only_codes:
            ...
            for row in range(50, dataset_labels.shape[0]):
                ...
                if len(row_indexes_by_patient) == 3:
                    ...
                    if all(k<X_tsne.shape[0] for k in rows_for_arrows):
                        ...
                        if (count_long_patients<5 and (end_patient_full_labels[0]==2 or end_patient_full_labels[0]==1)):
        ...
        plt.savefig(os.path.join(self.directory, "1_"+str(epoch)+"_progression_AIBL.png"))
        plt.close()
